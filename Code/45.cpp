//拓扑排序(基于有向无环图以及队列)
//拓扑排序可以用于实现得到某个集合全序，即将所有节点按优先级(箭头尾部的优先级高)先后进行输出(无相关性的节点或者说地位等价的节点自行规定输出)
//拓扑排序还可以用于判断有向无环图是否有环
#include <iostream>
using namespace std;
template <class T>
class node
...
template <class T>
tps<T>::tps(graph<T> &graph1)
{
    graphfortps = graph1;
}
template <class T>
void tps<T>::topologicalsort()
{
    queue<T> tempqueue;
    int count = 0;
    for (int i = 0; i < graphfortps.vexsize; i++)
    {
        ...
        {
            tempqueue.enqueue(graphfortps.vex[i].data);
        }
    }
    while (!tempqueue.empty())
    {
        ...
        for (adjvertex *w = graphfortps.vex[graphfortps.locate(temp)].firstadj; w; w = w->nextadj)
        {
            ...
            {
                tempqueue.enqueue(graphfortps.vex[w->adjvex].data);
            }
        }
    }
...
int main()
{
    graph<char> graph1;
    graph1.create(10,9);
    tps<char> tps1(graph1);
    tps1.topologicalsort();
    return 0;
}